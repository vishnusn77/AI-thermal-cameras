...
def detect_and_track_people(video_source="webcam", video_path=None, rtsp_url=None):
    ...
    while True:
        try:
            ret, frame = cap.read()
            if not ret:
                # Check if video source is a file and end gracefully if it has finished
                if video_source == "video":
                    print(f"Crowd limit is: {CROWD_THRESHOLD}")
                    print(f"Total people detected: {len(unique_ids)}")
                    break
                else:
                    print("Error: Could not read frame. Attempting to restart stream.")
                    cap.release()
                    cap = cv2.VideoCapture(rtsp_url) if video_source == "rtsp" else cap
                    continue
